...
class SproutTogetherSupabaseUser extends BaseAuthUser {
  SproutTogetherSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> sproutTogetherSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SproutTogetherSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
