// ─────────────────────────────────────────────────────────────────────────────
// PLANT IMAGE FALLBACKS — Wikipedia Commons verified images
// Every plant in the DB is mapped to a scientifically accurate image from
// Wikimedia Commons (upload.wikimedia.org CDN — loads fine in Flutter apps).
// Keys are lowercase plant_name values from Supabase.
// Restored from build 60 (commit febee2a) — this map was incorrectly deleted.
// ─────────────────────────────────────────────────────────────────────────────
...
const Map<String, String> kPlantImageFallbacks = {
  ...
  'clover (white / dutch)': 'https://upload.wikimedia.org/wikipedia/commons/thumb/5/52/Trifolium_repens_-_white_clover_on_way_from_Govindghat_to_Gangria_at_Valley_of_Flowers_National_Park_-_during_LGFC_-_VOF_2019_%281%29.jpg/600px-Trifolium_repens.jpg',
  ...
  'delphinium': 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/6d/Delphinium_elatum.jpg/600px-Delphinium_elatum.jpg',
...
/// Priority: kPlantImageFallbacks (Wikipedia Commons CDN) > Supabase DB URL.
/// The fallback map is checked first to guarantee correct plant images
/// regardless of what the DB image_url column contains.
String? bestPlantImageUrl(String? supabaseUrl, String? plantName) {
  // 1. Check kPlantImageFallbacks — plant-specific Wikipedia Commons image.
  if (plantName != null && plantName.isNotEmpty) {
    final key = plantName.toLowerCase().trim();
    if (kPlantImageFallbacks.containsKey(key)) {
      return kPlantImageFallbacks[key];
    }
    // Partial match for DB variants (e.g. "chamomile (german)" matches "chamomile").
  ...
  // 2. Trusted DB URL (Unsplash, Wikipedia Commons CDN, or Supabase storage).
  if (supabaseUrl != null &&
